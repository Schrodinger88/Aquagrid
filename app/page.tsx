...
export default function Home() {
  return (
    <div className="min-h-screen text-deep-forest font-sans selection:bg-kelp-green selection:text-white">

      {/* 
        HERO SECTION 
        Inspiration: Contained, heavily rounded hero image with overlay text.
      */}
      <section className="pt-8 px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto">
        <div className="relative w-full h-[75vh] min-h-[600px] rounded-[2.5rem] overflow-hidden shadow-2xl group">
          <Image
            ...
            priority
          />
          {/* Subtle gradient overlay to ensure text readability while keeping the cinematic image bright */}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 z-10">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 text-white px-5 py-2 rounded-full text-sm font-bold tracking-widest uppercase mb-8 shadow-sm flex items-center gap-2">
              <Leaf className="w-4 h-4 text-light-green" />
              Green Solutions for Blue Oceans
            </div>
            <h1 className="text-5xl md:text-7xl lg:text-[5.5rem] font-bold text-white mb-6 leading-[1.05] tracking-tight max-w-5xl drop-shadow-lg">
              Protecting Oceans, <br className="hidden md:block" />
              <span className="text-sky-blue/90">Preserving Our Future</span>
            </h1>
            <p className="text-lg md:text-xl text-white/90 max-w-2xl font-medium mb-10 drop-shadow-md leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/solutions" className="bg-kelp-green text-white hover:bg-[#187522] px-8 py-4 rounded-full font-bold transition-all shadow-xl shadow-kelp-green/20 hover:-translate-y-1 flex items-center gap-2 text-lg">
                Discover The Tech <ArrowRight className="w-5 h-5" />
              </Link>
              <Link href="https://cal.com/aquagrid" target="_blank" className="bg-white/10 backdrop-blur-sm border border-white/30 text-white hover:bg-white/20 px-8 py-4 rounded-full font-bold transition-all hover:-translate-y-1 text-lg">
                Book a Call directly
              </Link>
            </div>
          </div>

          {/* Floating bottom stat inside hero */}
          <div className="absolute bottom-6 right-6 hidden md:flex gap-4">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5 text-left shadow-2xl">
              <div className="text-sm text-white/70 uppercase tracking-wider font-bold mb-1">Impact per System</div>
              <div className="text-3xl font-bold text-white"><CountUpNumber end={1130} /> kg</div>
              <div className="text-sky-blue text-sm">CO₂ Absorbed / Year</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* 
        ...
        Clean typography, offset layouts, lots of whitespace.
      */}
      <section className="py-24 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row gap-16 items-center">
          ...
          <div className="md:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="bg-deep-forest text-white p-8 rounded-[2rem] shadow-xl relative overflow-hidden group hover:-translate-y-1 transition-all">
              ...
              <p className="text-white/70 font-medium leading-relaxed z-10 relative">A world where humanity and oceans coexist in balance, ensuring biodiversity and resources for generations to come.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              { icon: <Waves className="w-6 h-6 text-ocean-blue" />, title: "Declining Fisheries", desc: "Catch size and volume declines are already impacting our multi-million dollar shellfish market, threatening food security and coastal livelihoods." },
              ...
              <div key={i} className="glass-card p-10 group">
                <div className="w-14 h-14 bg-white/50 backdrop-blur-sm rounded-2xl flex items-center justify-center mb-6 shadow-sm group-hover:scale-110 transition-transform duration-300 border border-white/80">
                  {item.icon}
                </div>
      ...
      {/* 
        STAGGERED IMAGE & TEXT GRID - CREATIVE LAYOUT
      */}
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[300px]">
          ...
          <div className="md:col-span-8 relative rounded-[2.5rem] overflow-hidden group shadow-lg">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-deep-forest/80 to-transparent opacity-60" />
          ...
          {/* Dark Stat Card */}
          <div className="md:col-span-4 bg-deep-forest rounded-[2.5rem] p-10 flex flex-col justify-center text-white relative overflow-hidden group hover:bg-[#023106] transition-colors shadow-lg">
            ...
            <p className="text-white/70">Annually provided by global kelp forests through fisheries production and carbon sequestration.</p>
          </div>

          {/* Light Text Card */}
          <div className="md:col-span-4 glass-card p-10 flex flex-col justify-center group">
            <div className="w-12 h-12 bg-light-green rounded-full flex items-center justify-center mb-6 text-kelp-green font-bold border border-kelp-green/20 group-hover:bg-kelp-green group-hover:text-white transition-colors">
              01
            </div>
          ...
          {/* Smaller Stat Card */}
          <div className="md:col-span-3 glass-card bg-light-blue/40 p-10 flex flex-col justify-center relative group hover:-translate-y-2">
            <div className="text-4xl font-black text-ocean-blue mb-2"><CountUpNumber end={12.3} suffix="%" decimals={1} /></div>
      ...
      {/* 
        FINANCIALS / CTA FOOTER OFFSET
      */}
      <section className="py-24 bg-midnight text-white relative overflow-hidden mt-12 rounded-t-[3rem]">
        <div className="absolute -top-40 -left-40 w-96 h-96 bg-kelp-green rounded-full blur-[150px] opacity-40"></div>
        <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-sky-blue rounded-full blur-[200px] opacity-20"></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <Link href="/contact" className="bg-sky-blue text-deep-forest hover:bg-white px-8 py-4 rounded-full font-bold transition-all shadow-xl shadow-sky-blue/20 hover:-translate-y-1 text-lg flex items-center justify-center gap-2">
              Get Involved Today <ArrowRight className="w-5 h-5" />
            </Link>
            <Link href="https://cal.com/aquagrid" target="_blank" className="bg-white/10 text-white hover:bg-white/20 border border-white/20 px-8 py-4 rounded-full font-bold transition-all text-lg flex items-center justify-center">
